"""test_migrate_pythia_b1.py — B1 migráció: dry-run default, backup + apply,
idempotencia, verify (a migrate_pythia_p1 teszt-mintája)."""
import glob
import os
import sqlite3

import migrate_pythia_b1 as mig
...
def test_dry_run_default_writes_nothing(tmp_path, capsys):
    ...
    assert not glob.glob(db + ".pre_pythia_b1.*")   # backup sem készül
...
def test_apply_creates_tables_with_backup(tmp_path, capsys):
    ...
    backups = glob.glob(db + ".pre_pythia_b1.*")
...
def test_migration_ddl_matches_plugin_source_of_truth(tmp_path):
    ...
    (delphoi_brief.BRIEF_INIT_SQL) — nincs séma-kettősség."""
    db = _mk_db(tmp_path)
    assert mig.main(["--db", db, "--apply"]) == 0
    conn = sqlite3.connect(db)
    cols = {r[1] for r in conn.execute("PRAGMA table_info(delphoi_briefs)")}
    tcols = {r[1] for r in conn.execute("PRAGMA table_info(delphoi_tracking)")}
    rcols = {r[1] for r in conn.execute("PRAGMA table_info(delphoi_reports)")}
    conn.close()
    assert cols == {"brief_id", "user_id", "spec_json", "spec_hash", "created_at"}
    assert tcols == {"brief_id", "cadence", "next_run", "active"}
    assert rcols == {"id", "job_id", "format", "path", "created_at"}
