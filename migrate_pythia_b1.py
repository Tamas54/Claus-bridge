#!/usr/bin/env python3
"""migrate_pythia_b1.py — PYTHIA B1 brief-réteg migráció (Bridge).

Cél (GESAMTBEFEHL B1): a brief-réteg táblái éles DB-n, a migrate_pythia_p1.py
bevált mintájával (dry-run DEFAULT + időbélyeges backup + idempotens):
  - delphoi_briefs   (brief_id PK, user_id, spec_json, spec_hash, created_at)
  - delphoi_tracking (brief_id, cadence, next_run, active)
  - delphoi_reports  (job_id, format, path, created_at)
  - delphoi_brief_runs (idősor-pontok a tracking-riport fejezetéhez — a
    B1-parancs 3 táblájának természetes kiegészítése: a gördülő brief
    futásonkénti jele ITT gyűlik, a delphoi_jobs sor-hivatkozásával)

A DDL az EGYETLEN igazságforrásból jön: plugins.delphoi_brief.BRIEF_INIT_SQL
(IF NOT EXISTS — az újrafuttatás no-op). A meglévő táblákhoz NEM nyúlunk;
az append-only nowcast-ledger triggereit a CREATE TABLE nem érinti.

Futás:
  python migrate_pythia_b1.py                  # --dry-run a DEFAULT: csak terv
  python migrate_pythia_b1.py --apply          # backup + CREATE + verify
  python migrate_pythia_b1.py --db /path/x.db  # explicit DB (default: BRIDGE_DB_PATH | bridge.db)

--apply garanciák:
  1. időbélyeges FÁJL-MÁSOLAT backup a CREATE előtt;
  2. utána verify: mind a 4 tábla létezik + (ha van nowcast-ledger)
...
PLANNED_TABLES = ("delphoi_briefs", "delphoi_tracking", "delphoi_reports",
                  "delphoi_brief_runs")
...
def apply_plan(conn: sqlite3.Connection) -> None:
    ...
    from plugins.delphoi_brief import BRIEF_INIT_SQL
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="PYTHIA B1 brief-réteg migráció (idempotens CREATE-ek)")
    ...
    backup_path = f"{args.db}.pre_pythia_b1.{stamp}.bak"
