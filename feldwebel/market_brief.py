...
def _build_prompt(session: str, asof_iso: str, valid_until_iso: str, calendar_raw: str) -> str:
    ...
    return (
        ...
        "⚠️ AZ OKOT SEM TALALHATOD KI — CSAK A SZAMOT. Never attribute a market "
        "move to a NAMED person, speech, decision or event unless that event "
        "appears in the injected NEWS or DATA blocks. No 'the Fed chair said', "
        "no 'after X's remarks', no 'following the announcement' — if the data "
        "does not contain it, IT DID NOT HAPPEN as far as this brief is "
        "concerned. Describe WHAT the numbers are and, at most, what they are "
        "CONSISTENT with. And never let an invented cause drive `regime` or "
        "`risk_budget_pct`.\n"
        "(2026-08-31: a brief wrote 'a Fed-elnök Warsh inflációellenes "
        "retorikája nyomán emelkedő hozamok', set regime=risk_off and cut the "
        "risk budget to 25% — while the data it was given showed VIX 14.43, "
        "S&P near its 52-week high and crowd=complacent. Nothing about any Fed "
        "statement was in the input. The number was right; the story was "
        "invented, and the machine fields followed the story.)\n\n"
        "⚠️ A REZSIM A SZAMOKBOL KOVETKEZZEN. State in `note` WHICH figures "
        "drove `regime` and `risk_budget_pct` — and those figures must be in "
        "the data blocks. If the numbers are calm, do not write a dramatic "
        "brief to make it interesting.\n\n"
        ...
        "⚠️ A VALTOZATLANSAG UGYANUGY ALLITAS. \"ragadt\", \"stagnalt\", "
        "\"valtozatlan\", \"tovabbra is\", \"marad\" — ezek NEM semleges "
        "megfogalmazasok, hanem ket megfigyelest allitanak egyenlonek. Ugyanaz "
        "a szabaly all rajuk, mint a mozgast leiro szavakra. Ha csak egy szamod "
        "van, a helyes mondat: \"a maginflacio 3,7%\" — pont.\n"
        "(2026-08-31: a brief azt irta, 'a maginflacio 3,7%-on RAGADT'. A "
        "tenyleges sorozat: 2026-05 3,5 / 2026-06 3,5 / 2026-07 3,7 — tehat "
        "EMELKEDETT. Ugyanaznap az Echolot makro-szemleje helyesen irta, mert "
        "az megkapta az elozo erteket. Ket sajat termekunk mondott ellent "
        "egymasnak ugyanarrol a szamrol.)\n"
        "⚠️ HA VAN `time_series`, HASZNALD. A `get_macro_indicator` valasza "
        "gyakran hozza az elozo megfigyeleseket. Ne talalgass ott, ahol a "
        "valasz megmondja — es ne allits iranyt ott, ahol nem.\n\n"
